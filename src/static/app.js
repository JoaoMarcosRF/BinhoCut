...
document.addEventListener("DOMContentLoaded", function () {
  console.log("✓ BinhoCut inicializado");
...
function applyManualTimes() {
  ...
  if (endSeconds > videoInfo.duration) {
    alert(
      `❌ O tempo de fim (${formatTime(
        endSeconds
      )}) excede a duração do vídeo (${formatTime(videoInfo.duration)})`
    );
    return;
  }
  ...
  alert(
    `✅ Tempos aplicados com sucesso!\n\n⏮️ Início: ${formatTime(
      startSeconds
    )}\n⏭️ Fim: ${formatTime(endSeconds)}\n⏱️ Duração: ${formatTime(
...
function updateTimelineDisplay() {
  ...
  document.getElementById("time-duration").textContent = formatTime(
    ...
    try {
      ...
      if (data.done) {
        ...
        if (data.error) {
          ...
          alert(
            "⚠️ Nenhum clipe foi gerado. Verifique se o vídeo contém fala ou ajuste as configurações."
          );
          resetUI();
        }
      }
...
function showResults(result) {
  ...
  result.files.forEach((file, index) => {
    ...
    card.innerHTML = `
            ...
            <div style="margin-top:1rem; display: flex; flex-wrap: wrap; gap: 0.5rem;">
                ...
                <a href="/done/${file.replace(
                  ".mp4",
                  "_post.txt"
                )}" download class="btn btn-secondary">📱 Post Instagram</a>
                <a href="/done/${file.replace(
                  ".mp4",
                  "_analise.txt"
                )}" download class="btn btn-secondary">📊 Insights</a>
            </div>
        `;
  ...
  if (result.analytics) {
    ...
    document.getElementById("stat-clips").textContent =
      result.analytics.total_clips;
    document.getElementById("stat-score").textContent = Math.round(
      result.analytics.avg_score
    );
    document.getElementById("stat-duration").textContent =
      Math.round(result.analytics.total_duration) + "s";
    document.getElementById("stat-sentiment").textContent =
      result.analytics.sentiment.sentiment;
    document.getElementById("stat-time").textContent =
      Math.round(result.analytics.processing_time || 0) + "s";
  }
...
function getPreferences() {
  return {
    subtitle_font:
      document.getElementById("subtitle-font")?.value || "Courier-New-Bold",
    subtitle_size: parseInt(
      document.getElementById("subtitle-size")?.value || 70
    ),
    with_subtitles: document.getElementById("with-subtitles")?.checked || true,
    video_speed: parseFloat(
      document.getElementById("video-speed")?.value || 1.0
    ),
...
async function loadSettings() {
  try {
    ...
    if (document.getElementById("subtitle-size")) {
      document.getElementById("subtitle-size").value =
        prefs.subtitle_size || 70;
    }
    ...
    if (document.getElementById("subtitle-font")) {
      document.getElementById("subtitle-font").value =
        prefs.subtitle_font || "Courier-New-Bold";
    }
    if (document.getElementById("with-subtitles")) {
      document.getElementById("with-subtitles").checked =
        prefs.with_subtitles !== false;
    }
  } catch (error) {
...
async function loadHistory() {
  try {
    ...
    if (!history || history.length === 0) {
      container.innerHTML =
        '<p style="text-align:center;padding:2rem;color:rgba(255,255,255,0.5)">📭 Nenhum processamento ainda.</p>';
      return;
    }
    ...
    history.forEach((item) => {
      ...
      div.innerHTML = `
                <div style="color:rgba(255,255,255,0.6);font-size:0.85rem">${date.toLocaleString(
                  "pt-BR"
                )}</div>
                ...
                <p style="color:rgba(255,255,255,0.7)">
                    ${item.clips_count} clipes • Score: ${Math.round(
        item.avg_score
      )}
                </p>
...
async function loadDashboard() {
  try {
    ...
    document.getElementById("avg-duration-stat").textContent =
      Math.round(data.avg_duration || 0) + "s";
...
console.log("✓ BinhoCut AI carregado com sucesso");
